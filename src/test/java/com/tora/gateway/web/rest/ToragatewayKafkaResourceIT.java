...
import com.tora.gateway.config.KafkaProperties;
...
class ToragatewayKafkaResourceIT {
    ...
    void setup() {
        ...
        ToragatewayKafkaResource kafkaResource = new ToragatewayKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/toragateway-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/toragateway-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
